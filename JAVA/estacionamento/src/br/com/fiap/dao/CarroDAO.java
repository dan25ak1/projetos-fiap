...
public class CarroDAO {
    ...
    public String inserir(Carro carro) {
        String sql = "insert into ddd_carro(placa,cor,descricao) values(?,?,?)"; // numero de interrogaçoes de acordo com colunas que queremos preencher
    ...
    public String alterar(Carro carro) {
        String sql = "update ddd_carro  set cor=?, descricao=? where placa=?"; // numero de interrogaçoes de acordo com colunas que queremos preencher
        try(PreparedStatement ps = getCon().prepareStatement(sql);) {
    ...
    public String excluir(Carro carro) {
        String sql = "delete from ddd_carro where placa=?"; // numero de interrogaçoes de acordo com colunas que queremos preencher
        try(PreparedStatement ps = getCon().prepareStatement(sql);) {
            ps.setString(1, carro.getPlaca());
            if (ps.executeUpdate() > 0) {
